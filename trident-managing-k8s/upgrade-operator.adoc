...
Nell'ambito di questa fase, l'operatore 21.07.1 Trident identificherà un'installazione Astra Trident esistente e la aggiornerà alla stessa versione dell'operatore.
...
NOTE: Se viene visualizzato lo stato `Updating`, assicurarsi di risolverlo prima di procedere. Per un elenco dei possibili valori di stato, vedere link:../trident-kubernetes/kubernetes-deploy-operator.html["qui"^].
